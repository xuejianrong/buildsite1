...
class ContentItem extends \common\lib\DbOrmModel{
	const STATUS_NOT_PUBLISH = 0;	//未发布
	const STATUS_PUBLISHED = 1;		//已发布
	const STATUS_DELETE = 2;		//已删除
	
	const TYPE_CUSTOMER_CASE = 1;	//客户案例
	
	protected $_aEncodeFields = ['other_info' => 'json'];
	
	public static function tableName(){
		return Yii::$app->db->parseTable('_@content_item');
	}
	
	public static function getTypeList(){
		return [
			static::TYPE_CUSTOMER_CASE => '客户案例',
		];
	}
	...
	private static function _parseWhereCondition($aCondition = []){
		...
		if(isset($aCondition['is_jinping'])){
			$aWhere[] = ['is_jinping' => $aCondition['is_jinping']];
		}
