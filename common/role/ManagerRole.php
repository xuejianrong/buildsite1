...
use \common\model\Manager as ManagerModel;

/**
 * 用户登陆控制类
 */
class ManagerRole extends \yii\web\User{
	...
	public function login(\yii\web\IdentityInterface $oUser, $isRemember = false){
	 ...
	 private function _setLoginCookie($userId){
		Cookie::setEncrypt('managerId', $userId . ':' . NOW_TIME, NOW_TIME + $this->rememberLoginTime);
	}

	private function _getLoginUserIdFromCookie(){
		$str = Cookie::getDecrypt('managerId');
		if($str){
			$aData = explode(':', $str);
			return (int)$aData[0];
		}
	...
	private function _setLoginSession($userId){
		Yii::$app->session->set('managerId', $userId . ':' . NOW_TIME);
	}

	private function _getLoginUserIdFromSession(){
		$str = Yii::$app->session->get('managerId');
		if($str){
			$aData = explode(':', $str);
			return (int)$aData[0];
		}
	...
	public function logout($destroySession = true){
		...
		Yii::$app->session->remove('managerId');
